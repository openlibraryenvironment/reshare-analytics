--metadb:function rtat_reqs

DROP FUNCTION IF EXISTS rtat_reqs;

CREATE FUNCTION rtat_reqs()
RETURNS TABLE(
    rtr_requester text,
    rtr_start timestamptz,
    rtr_requester_nice_name text,
    rtr_hrid text,
    rtr_title text,
    rtr_call_number text,
    rtr_barcode text,
    rtr_supplier text,
    rtr_supplier_nice_name text,
    rtr_date_created timestamp,
    rtr_id uuid)
AS $$
SELECT DISTINCT pr.__origin AS rtr_requester,
    ...
    FROM reshare_rs.patron_request AS pr
        ...
        LEFT JOIN reshare_rs.symbol AS s2 ON pr.pr_resolved_sup_inst_symbol_fk::uuid = s2.sym_id AND pr.__origin = s2.__origin
        LEFT JOIN reshare_rs.directory_entry AS de ON s.sym_owner_fk = de.de_id AND s.__origin = de.__origin
        LEFT JOIN reshare_rs.directory_entry AS de2 ON s2.sym_owner_fk = de2.de_id AND s2.__origin = de2.__origin
    WHERE pr.pr_is_requester IS TRUE AND pr.pr_hrid IS NOT NULL
$$
LANGUAGE SQL
STABLE
PARALLEL SAFE;
