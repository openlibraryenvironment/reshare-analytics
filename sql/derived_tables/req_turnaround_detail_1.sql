...
CREATE TABLE rtat_reqs AS SELECT DISTINCT
    ...
    s2.de_name AS rtr_requester_nice_name,
    ...
    s3.de_name AS rtr_supplier_nice_name,
...
FROM
    ...
    LEFT JOIN symbol_directory_entry AS s2 ON pr.pr_resolved_req_inst_symbol_fk::uuid = s2.sym_id
    LEFT JOIN symbol_directory_entry AS s3 ON pr.pr_resolved_sup_inst_symbol_fk::uuid = s3.sym_id
WHERE
